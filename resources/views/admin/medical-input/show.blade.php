            ...
            <div class="card-body px-4 px-md-5 pb-5">
                <div class="row">
                    ...
                    <div class="col-md-4 text-center">
                        @if ($medical->patient->picture)
                            <img src="{{ asset('storage/' . $medical->patient->picture) }}" alt="Patient Picture"
                    ...
                    <div class="doctor-comments mt-4">
                        <div class="d-flex justify-content-between">
                            ...
                            <a href="{{ route('admin.edit.comments', $medical->id) }}" class="btn btn-warning">
                                Update Comments
                            </a>
                            <div>
                                @if (
                                    !empty($medical->interpretation_comments) ||
                                        !empty($medical->recommendations) ||
                                        !empty($medical->prescriptions) ||
                                        !empty($medical->result_summary))
                                    <a href="{{ route('admin.edit.comments', $medical->id) }}" class="btn btn-warning">
                                        Update Comments
                                    </a>
                                @endif
                        ...
                        @if (
                            ...
                            <p class="card-text"><strong>Interpretations:</strong>
                                {{ $medical->interpretation_comments }}
                            </p>
                            <p class="card-text"><strong>Recommendations:</strong> {{ $medical->recommendations }}</p>
                            <p class="card-text"><strong>Prescriptions:</strong> {{ $medical->prescriptions }}</p>
                            <p class="card-text"><strong>Result summary:</strong> {{ $medical->result_summary }}</p>
                            <p class="card-text"><strong>Comments Date Taken:</strong>
                                {{ \Carbon\Carbon::parse($medical->created_at)->format('F j, Y') }}</p>
                        @else
                            <div>
