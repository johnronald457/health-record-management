        ...
        <div class=" shadow">
            ...
            <div class="card-body px-5 pb-5">
                <div class="health-assessment">
                    <h5 class="card-text"><strong>Patient Name:</strong> {{ $treatment->user->firstname }}
                        {{ $treatment->user->lastname }}</h5>
                    <hr>
                    <div class="d-flex justify-content-between">
                        <h4 class="card-text mb-3"><strong>Assessment Details:</strong></h4>
                        <div>
                            <a href="{{ route('nurse.health.edit', $treatment->health_assessment->id) }}"
                    ...
                    <p class="card-text"><strong>Blood pressure:</strong>
                        {{ $treatment->health_assessment->blood_pressure }}</p>
                    <p class="card-text"><strong>Heart rate:</strong> {{ $treatment->health_assessment->heart_rate }}</p>
                    <p class="card-text"><strong>Medical Condition:</strong>
                        {{ $treatment->health_assessment->medical_conditions }}</p>
                    <p class="card-text"><strong>Medical History:</strong>
                        {{ $treatment->health_assessment->medical_history }}</p>
                    ...
                    <p class="card-text"><strong>Assessment Date Taken:</strong>
                ...
                <div class="doctor-comments mt-4">
                    ...
                    @if (
                        $treatment->interpretation_comments ||
                            ...
                            $treatment->result_summary)
                        <p class="card-text"><strong>Interpretations:</strong>
                            {{ $treatment->interpretation_comments ?? 'No comment yet' }}</p>
                        <p class="card-text"><strong>Recommendations:</strong>
                            {{ $treatment->recommendations ?? 'No comment yet' }}</p>
                        <p class="card-text"><strong>Prescriptions:</strong>
                            {{ $treatment->prescriptions ?? 'No comment yet' }}</p>
                        <p class="card-text"><strong>Result summary:</strong>
                            {{ $treatment->result_summary ?? 'No comment yet' }}</p>
                        <p class="card-text"><strong>Comments Date Taken:</strong>
                            {{ \Carbon\Carbon::parse($treatment->created_at)->format('F j, Y') }}</p>
                    @else
                        <p class="card-text">No doctor comments available...</p>
                    @endif
                </div>
            </div>
        </div>
